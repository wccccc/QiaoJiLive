...
import com.google.android.exoplayer2.ext.rtmp.io.RtmpSessionInfo;
...
public class RtmpHeader {
	...
	/**
	 * RTMP packet/message type definitions. Note: docstrings are adapted from
	 * the official Adobe RTMP spec: http://www.adobe.com/devnet/rtmp/
	 */
	public enum MessageType {

		/**
		 * Protocol control message 1 Set Chunk Size, is used to notify the peer
		 * a new maximum chunk size to use.
		 */
		SET_CHUNK_SIZE(0x01),
		/**
		...
		ABORT(0x02),
		/**
		 * Protocol control message 3 The client or the server sends the
		 * acknowledgment to the peer after receiving bytes equal to the window
		 * size. The window size is the maximum number of bytes that the sender
		 * sends without receiving acknowledgment from the receiver.
		 */
		ACKNOWLEDGEMENT(0x03),
		/**
		 * Protocol control message 4 The client or the server sends this
		 * message to notify the peer about the user control events. This
		 * message carries Event type and Event data. Also known as a PING
		 * message in some RTMP implementations.
		 */
		USER_CONTROL_MESSAGE(0x04),
		/**
		 * Protocol control message 5 The client or the server sends this
		 * message to inform the peer which window size to use when sending
		 * acknowledgment. Also known as ServerBW ("server bandwidth") in some
		 * RTMP implementations.
		 */
		WINDOW_ACKNOWLEDGEMENT_SIZE(0x05),
		/**
		 * Protocol control message 6 The client or the server sends this
		 * message to update the output bandwidth of the peer. The output
		 * bandwidth value is the same as the window size for the peer. Also
		 * known as ClientBW ("client bandwidth") in some RTMP implementations.
		 */
		SET_PEER_BANDWIDTH(0x06),
		/**
		 * RTMP audio packet (0x08) The client or the server sends this message
		 * to send audio data to the peer.
		 */
		AUDIO(0x08),
		/**
		 * RTMP video packet (0x09) The client or the server sends this message
		 * to send video data to the peer.
		 */
		VIDEO(0x09),
		/**
		 * RTMP message type 0x0F The client or the server sends this message to
		 * send Metadata or any user data to the peer. Metadata includes details
		 * about the data (audio, video etc.) like creation time, duration,
		 * theme and so on. This is the AMF3-encoded version.
		 */
		DATA_AMF3(0x0F),
		/**
		 * RTMP message type 0x10 A shared object is a Flash object (a
		 * collection of name value pairs) that are in synchronization across
		 * multiple clients, instances, and so on. This is the AMF3 version:
		 * kMsgContainerEx=16 for AMF3.
		 */
		SHARED_OBJECT_AMF3(0x10),
		/**
		 * RTMP message type 0x11 Command messages carry the AMF-encoded
		 * commands between the client and the server. A command message
		 * consists of command name, transaction ID, and command object that
		 * contains related parameters. This is the AMF3-encoded version.
		 */
		COMMAND_AMF3(0x11),
		/**
		 * RTMP message type 0x12 The client or the server sends this message to
		 * send Metadata or any user data to the peer. Metadata includes details
		 * about the data (audio, video etc.) like creation time, duration,
		 * theme and so on. This is the AMF0-encoded version.
		 */
		DATA_AMF0(0x12),
		/**
		 * RTMP message type 0x14 Command messages carry the AMF-encoded
		 * commands between the client and the server. A command message
		 * consists of command name, transaction ID, and command object that
		 * contains related parameters. This is the common AMF0 version, also
		 * known as INVOKE in some RTMP implementations.
		 */
		COMMAND_AMF0(0x14),
		/**
		 * RTMP message type 0x13 A shared object is a Flash object (a
		 * collection of name value pairs) that are in synchronization across
		 * multiple clients, instances, and so on. This is the AMF0 version:
		 * kMsgContainer=19 for AMF0.
		 */
		SHARED_OBJECT_AMF0(0x13),
		/**
		 * RTMP message type 0x16 An aggregate message is a single message that
		 * contains a list of sub-messages.
		 */
		...
		public static MessageType valueOf(byte messageTypeId) throws IOException {
			if (quickLookupMap.containsKey(messageTypeId)) {
				...
				// byte: " + Util.toHexString(messageTypeId));
			}
		}
	}
	...
	public enum ChunkType {
		...
		/**
		 * Relative 1-byte RTMP chunk header (no "real" header, just the 1-byte
		 * indicating chunk header type & chunk stream ID)
		 */
		...
		private byte value;
		/**
		 * The full size (in bytes) of this RTMP header (including the basic
		 * header byte)
		 */
		...
		public static ChunkType valueOf(byte chunkHeaderType) throws IOException {
			if (quickLookupMap.containsKey(chunkHeaderType)) {
				return quickLookupMap.get(chunkHeaderType);
			} else {
				throw new IOException("Unknown chunk header type byte: " + Util.toHexString(chunkHeaderType));
				// throw new IllegalArgumentException("Unknown chunk header type
				// byte: " + Util.toHexString(chunkHeaderType));
			}
		}
	}
...
//		}
	...
	private void readHeaderImpl(InputStream in, RtmpSessionInfo rtmpSessionInfo) throws IOException {
		...
		// Log.e(TAG,
		// "basicHeaderByte:"+Util.toHexString((byte)basicHeaderByte));
		...
		case TYPE_0_FULL: { // b00 = 12 byte header (full header)
			...
			// Log.e(TAG, "absoluteTimestamp:"+absoluteTimestamp);
			timestampDelta = 0;
			// Read bytes 4-6: Packet length
			packetLength = Util.readUnsignedInt24(in);
			// Log.e(TAG, "packetLength:"+packetLength);
			...
			try {
				...
				in.read(b);
				for (int i = 0; i < 30; i++) {
					Log.e(TAG, " " + Util.toHexString(b[i]));
				}
				throw e;
			}
			// Read bytes 8-11: Message stream ID (apparently little-endian
			// order)
		...
		case TYPE_1_RELATIVE_LARGE: { // b01 = 8 bytes - like type 0. not
										// including message stream ID (4 last
										// bytes)
		...
		case TYPE_2_RELATIVE_TIMESTAMP_ONLY: { // b10 = 4 bytes - Basic Header
												// and timestamp (3 bytes) are
												// included
			// Read bytes 1-3: Timestamp delta
			timestampDelta = Util.readUnsignedInt24(in);
			// Read bytes 1-4: Extended timestamp delta
			...
			RtmpHeader prevHeader = rtmpSessionInfo.getChunkStreamInfo(chunkStreamId).prevHeaderRx();
			if (prevHeader == null) {
				throw new IOException("Unknown chunk stream id " + chunkStreamId);
				// throw new IllegalArgumentException("Unknown chunk stream id "
				// + chunkStreamId);
			}
		...
		default:
			throw new IOException(
					"Invalid chunk type; basic header byte was: " + Util.toHexString((byte) basicHeaderByte));
		}
//		Log.e(TAG, "chunkType:"+chunkType);
//		Log.e(TAG, "absoluteTimestamp:"+absoluteTimestamp);
//		Log.e(TAG, "timestampDelta:"+timestampDelta);
//		Log.e(TAG, "extendedTimestamp:"+extendedTimestamp);
...
//		Log.e(TAG, "-------------------------------------------");
	...
	public void writeTo(OutputStream out, ChunkType chunkType, final ChunkStreamInfo chunkStreamInfo)
		...
		case TYPE_1_RELATIVE_LARGE: { // b01 = 8 bytes - like type 0. not
										// including message ID (4 last bytes)
	...
	private void parseBasicHeader(byte basicHeaderByte) throws IOException {
		chunkType = ChunkType.valueOf((byte) ((0xff & basicHeaderByte) >>> 6)); // 2
																				// most
																				// significant
																				// bits
																				// define
																				...
																				// chunk
																				// type
		chunkStreamId = basicHeaderByte & 0x3F; // 6 least significant bits
												// define chunk stream ID
	}
