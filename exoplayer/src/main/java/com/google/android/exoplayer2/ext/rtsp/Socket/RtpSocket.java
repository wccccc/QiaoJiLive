package com.google.android.exoplayer2.ext.rtsp.Socket;
...
public class RtpSocket implements Runnable {
    ...
    public RtpSocket(boolean isTcptranslate, int port, String ip, int serverPort,int trackType) throws IOException {
        Log.e(tag,"new RtpSocket port:"+port);
        this.port = port;
        this.ip = ip;
...
//            mRtcpSocket = null;
    ...
    public void sendUDPPacket(byte[] data){
		try {
			...
			Log.e(tag, "send upd packet fail!");
			e.printStackTrace();
		} catch (IOException e) {
			Log.e(tag, "send upd packet fail!");
			e.printStackTrace();
