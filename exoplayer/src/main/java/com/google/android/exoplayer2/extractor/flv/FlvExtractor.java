...
public final class FlvExtractor implements Extractor, SeekMap {
  ...
  private static final String TAG="FlvExtractor";
  ...
  public FlvExtractor(TimestampAdjuster timestampAdjuster){
	  this();
  ...
  private boolean readTagHeader(ExtractorInput input) throws IOException, InterruptedException {
    ...
    if(tagType==TAG_TYPE_AUDIO||tagType==TAG_TYPE_VIDEO){
    	if(firstTimestamp==-1){
    		firstTimestamp=tagTimestampUs;
        }
    	tagTimestampUs-=firstTimestamp;
    }
    if(timestampAdjuster!=null){
    	tagTimestampUs+=lastTimestamp;
    }
//    if(tagType==TAG_TYPE_AUDIO){
//    	 Log.e(TAG, "audio:"+tagTimestampUs);
//    }else if(tagType==TAG_TYPE_VIDEO){
//    	 Log.e(TAG, "video:"+tagTimestampUs);
//    }
