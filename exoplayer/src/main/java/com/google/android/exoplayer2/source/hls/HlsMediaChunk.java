...
/* package */ final class HlsMediaChunk extends MediaChunk {
  ...
  private static final String FLV_FILE_EXTENSION = ".flv";
  ...
  private Extractor createExtractor() {
    ...
    if (MimeTypes.TEXT_VTT.equals(hlsUrl.format.sampleMimeType)
      ...
      if(extractor instanceof FlvExtractor){
        ((FlvExtractor)extractor).reset();
      }
    } else if (lastPathSegment.endsWith(MP4_FILE_EXTENSION)
        || lastPathSegment.startsWith(M4_FILE_EXTENSION_PREFIX, lastPathSegment.length() - 4)) {
      extractor = new FragmentedMp4Extractor(0, timestampAdjuster);
    }else if (lastPathSegment.endsWith(FLV_FILE_EXTENSION)){
      extractor=new FlvExtractor(timestampAdjuster);
    }else{
