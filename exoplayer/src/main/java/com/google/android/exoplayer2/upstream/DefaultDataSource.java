...
public final class DefaultDataSource implements DataSource {
  ...
  public long open(DataSpec dataSpec) throws IOException {
    ...
    } else if (SCHEME_RTMP.startsWith(scheme)) {
      dataSource = getRtmpDataSource();
    } else if (SCHEME_RTSP.startsWith(scheme)){
  ...
  private DataSource getRtspDataSource(){
    if (rtspDataSource == null) {
      try {
        Class<?> clazz = Class.forName("com.google.android.exoplayer2.ext.rtsp.RtspDataSource");
